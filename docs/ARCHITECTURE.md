...
**The endpoint widened on 2026-09-05, and the rule it widened under is the important part.** Steward's Finance page was one band of three figures beside the subscriptions, while Horizon computed a great deal it never saw. What crossed: the balance sheet's four scalars, registered-account room, and the dividend summary — **plus `totalCostCents`, `cashCadCents` and `cashUsdCents`, which the route had been computing on every call and discarding.**

**Aggregates only, and one of those calls had to be trimmed to stay that way.** `computeBalanceSheet` also returns `manualAssetRows` and `liabilityRows`, which carry "Maison — <address>" and "Prêt privé — <name>". They are destructured away in the route rather than spread, so a field added to them later cannot arrive here by accident. Contribution room and the dividend summary have no identifiers in their return types at all. What stays behind for the same reason: day movers and top yielders, both of which name individual securities.

**The correction that made it worth doing.** Horizon's `netWorthCents` is positions plus cash — it counts neither the house nor any liability, and Horizon's own comment says the name is wrong. Steward rendered it under a heading called *Portfolio* and therefore never held the figure a person means by net worth. The wire name is unchanged, because renaming it would break a deployed Steward mid-flight; `trueNetWorthCents` sits beside it and the page leads with that.

**Every new field is nullable and reads as absent, never as zero.** A Steward deployed before Horizon gets a payload without them, and that is a state this deploy passes through by construction — a confident $0 net worth is precisely the failure rule 2 exists for.

**The exchange rate rides along on the same response**, added 2026-09-02 as `usdCadRate` and `fxDate`. It is not a portfolio figure — every number beside it is already CAD — and it is there because a subscription can be billed in US dollars and Steward has to show what that costs in Canadian ones. Horizon has pulled `USDCAD=X` from Yahoo into an `FxRate` table five times every weekday since long before Steward asked, so a second endpoint, a second `SourceKey` and a second nightly job to carry one slow-moving number would all have been waste. The cost is one coupling worth naming: when the summary fails, the rate goes stale with it, and the Finance panel is already amber at that point.

**Steward converts or it says it cannot.** Horizon's own `getLatestFxRate` returns 1.0 when it has no rate, logging a warning — US dollars quietly counted as Canadian ones. That is a fair trade there and forbidden here: a monthly total is what Vincent budgets against, and one silently a third too low is the failure rule 2 exists to prevent. So `lib/fx.ts` returns null rather than a number it cannot stand behind, an unconvertible row is excluded from the total **and counted**, and the page names the gap. Canadian rows convert with no rate at all, because there is nothing to convert and a missing rate must not blank out the majority of the list.
...
**The folders are found by `specialUse`, never by name.** Gmail localises them — on a French account Trash is `[Gmail]/Corbeille` — so a hardcoded `[Gmail]/Trash` would fail in the worst way available, by reporting success while moving mail somewhere unexpected or nowhere.

**A mail row also carries what the message says and where it wants you to go.** Both are extracted by the summarise job from a body it already downloads, so they cost no extra IMAP work: links come off the **raw** download before the markup is stripped, so one pass catches an HTML `href` and a bare URL alike. **Two rules rank them** — the sender's own registrable domain first, then repetition, because `links.email.steampowered.com` shares Steam's domain and would otherwise beat `store.steampowered.com` on document order. A real destination appears on every call to action; "view this in your browser" appears once. That is a property of how marketing mail is written rather than a list of hosts to distrust, so it does not go stale.

Both land on `Item.detail`, which forces one thing: **the collector writes `detail` on `create` only**, or its five-minute upsert would wipe the job's work. Safe, because all it writes there is the sender's address and a message's sender does not change.
