...
**Each tile shows its area's most pressing fact, not a fixed metric.** Finance reads the day's change on an ordinary day and names a renewal when one is inside its notice window, because that is then the more pressing thing about money. Systems reads `8/8 up`, or names the worst thing — `disk4 disabled`, `Plex is down` — or rolls up to `3 to check` when there are several.

**The dot is the state and the tint is the emphasis.** A fine area is a plain card with a teal dot; an area that wants something takes a tinted ground, a matching dot and coloured text on its fact. The area's name stays `text-faint` in both cases: the name is a label and the fact is the urgent part. The vocabulary is the rail's own — `Dot` already speaks `ok / down / stale / degraded` on `/systems` and in the sidebar — plus `due` in gold, added for this, meaning a deadline that is not a fault.

**Eleven tiles became five on 2026-09-02**, on Vincent's verdict that the band was a mess, that the colour code was not doing its job and that most tiles were unclear — *"3 today", "0 late": what does it refer to, is it good or bad?* Three faults, compounding. **Three tiles repeated the cards two inches away**: `3 today`, `5 due today` and `0 late` were the Late, Today and Upcoming cards in the same viewport, which is the redundancy argument that removed the gate card broken three times over — and the tell was that those were precisely the three tiles that could link nowhere. **Labels did not name their subject**, so `today` and `close` and `to file` each parsed only if you already knew the area. And **colour had no way to say "fine"**: eight of eleven tiles were permanently neutral and the rest lit only on failure, so a normal day was eleven identical grey chips.

**`unavailable` went with them.** A number with no threshold that never lights is not an overview, it is furniture. **Mail's threshold is a stated guess** — above 25 read messages still in the inbox it asks for attention — because Vincent asked to be told whether a number is good or bad and a threshold is the only honest way to answer that. It reverses a "never coloured" decision made one commit earlier, and the code says so.

**Mail is the one tile that leaves Steward**, to the Gmail inbox: filing is done there and the queue is unread mail, not this. **Documents and Launcher still get nothing**: both are reference surfaces, and a tile that can never change is dead width in a strip whose whole job is to be scanned.

**The band fits as many tiles per row as it can.** `repeat(auto-fit, minmax(180px, 1fr))` — a wider floor than the eleven-tile band's 132px, since the tiles are richer. Still not a written-down count: three of the five are conditional, so any fixed number can orphan one.
...
- **Systems page**: two bands and a footer, from the `TabSystems` artboard. *Services* fills the width with a **grid** of monitors — `auto-fill` at a 210px minimum rather than the artboard's fixed six across, because the monitor count is Vincent's to change in Uptime Kuma and the page should not care how many there are. Then a row of *Server*, *WhiteTower* and *Home Assistant* — the artboard drew two, and the machine joined its own array on 2026-09-01. Then *Collectors*, **eight across at `2xl` — one column per collector**, seven until Gmail joined on 2026-09-01. It shares Services' ladder up to `lg` and diverges above it on purpose: Services counts whatever is in Uptime Kuma and must stay responsive to a number set elsewhere, while the collectors are the adapter list, fixed in code, so a grid that fits them exactly leaves no orphan on a second row. A ninth means changing the number, which is cheaper than the orphan — and the eighth also moved the step from `xl` to `2xl`, because eight columns at `xl` is 126px each once the rail is subtracted and the labels truncate. Then the links out.
  ...
  **Uptime is drawn from 2026-09-03**, having been ruled out here since the mockup. The old reasoning stands as far as it went — `/metrics` carries no incident history, and `changedAt` is only ever when Steward watched it change — but it was an argument about what Kuma will tell Steward, not about what Steward can watch for itself. It now keeps its own outage rows and the card reports from those.

  **The card names the window it actually watched.** "No outages in 6 hours" rather than a percentage over a month nobody observed, and a percentage only once there is something to report. Immediately after a deploy every card says a few minutes, which is the honest thing and looks it.

  A down service **tints**: ground, border and caption in the tone its dot already carried. It used to be a red dot over a gold caption on an ordinary card — three opinions about one fact. And a service card is **a link to the service**, which `Monitor.url` had made possible since the first day.
