...
- **Today card**: the schedule, then **Late**, **Due today** and **Upcoming**, each with an 11px uppercase label and its count, then the standing facts of the day — supper, bins, tomorrow's school day. It shipped as one date-ordered task list with a small "late" tag per row and a sentence at the bottom counting them, which buried the most actionable thing on the card inside the least. Only *Late* carries a colour, because "these have already slipped" is the one meaning on this card that has one; *Upcoming* is muted, because at full weight a busy week out-shouts the two things actually due.
