...
- ~~**Uptime duration or percentage on a service tile.**~~ **Done 2026-09-03**, and the entry is kept because how it was wrong is worth more than that it is closed. It read: *"neither is reachable on the current read path… it needs a second read path into Kuma, and that is a deliberate exception to be argued rather than slipped in"*, on the grounds that `/api/badge/<id>/uptime/24` needs a monitor id `/metrics` does not expose and socket.io is ruled out by rule 6.

  Every one of those facts is still true, and the conclusion did not follow from them. They are an argument about **what Uptime Kuma will tell Steward**, and the question was what Steward can know — which is a different thing, because the adapter was already detecting every up↔down transition in order to move `changedAt`. Writing two rows per outage at that same line gives a real figure with no new read path at all. The cost is that it cannot backfill, which `Monitor.watchedSince` makes the card say out loud rather than hide.

  The certificate series was closed at the same time, and by the same kind of correction: it had been ignored as "everything the source happens to offer", which was right while nothing rendered it.
